...
data.raw <- read.csv("B1.csv", as.is=TRUE, header=TRUE)
...
max.pts <- 245 # Maximum points plotted
...
plot_new <- ggplot(data_new, aes(x=row_n, y=A)) +
        ...
        #geom_vline(xintercept = 1713, lty = 2, lwd=1,colour = "blue") +
        theme_bw() +
        ...
        #annotate("text", x = 1743, y = 34.5, label = "2016")
plot_new
